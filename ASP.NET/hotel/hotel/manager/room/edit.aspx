<%@ Page Language="C#" AutoEventWireup="true" CodeFile="edit.aspx.cs" Inherits="manager_room_edit" %>
        ...
        <td>
            <select name="room_status">
                     <%
                         ...
                         if (status == "empty")
                         {
                             Response.Write("<option value =\"empty\" selected=\"selected\">empty</option>");
                             Response.Write("<option value =\"checked\">checked</option>");
                             Response.Write("<option value =\"live\">live</option>");
                         }
                         else if (status == "checked")
                         {
                             Response.Write("<option value =\"empty\">empty</option>");
                             Response.Write("<option value =\"checked\" selected=\"selected\">checked</option>");
                             Response.Write("<option value =\"live\">live</option>");
                         ...
                         else if (status == "live")
                         {
                             Response.Write("<option value =\"empty\">empty</option>");
                             Response.Write("<option value =\"checked\">checked</option>");
                             Response.Write("<option value =\"live\" selected=\"selected\">live</option>");
    ...
    <tr>
        ...
        <select name="type_id">
            <%
                ...
                string type_id = Request.QueryString["tid"];
                if (ds != null || ds.Tables.Count != 0 || ds.Tables[0].Rows.Count != 0)
                {
                    for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                    {
                        ...
                        if (type_id == tid)
                        {
                            Response.Write("<option value =\"" + tid + "\" selected=\"selected\">" + type_name + "</option>");
                        }
                        else
                        {

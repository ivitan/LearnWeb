...
6. 在BulletedList中追加数据项
    ...
    <div id="div_hidden">
        <asp:AccessDataSource ID="ADS_L_SE" runat="server" DataFile="~/mdb/EV.mdb" SelectCommand="SELECT se_name, se_id FROM T_SERIES WHERE (se_enable = true) AND (se_delete = false) ORDER BY se_order DESC">
        </asp:AccessDataSource>
        <asp:AccessDataSource ID="ADS_L_SI" runat="server" DataFile="~/mdb/EV.mdb" SelectCommand="SELECT si_id, si_name FROM T_SIZE WHERE (si_enable = true) AND (si_delete = false) ORDER BY si_order DESC">
        </asp:AccessDataSource>
        <asp:AccessDataSource ID="ads_lv" runat="server" DataFile="~/mdb/EV.mdb" SelectCommand="SELECT TB_MONITOR.mo_id, TB_MONITOR.mo_seid, TB_MONITOR.mo_siid, TB_MONITOR.mo_imgid, TB_MONITOR.mo_name, TB_MONITOR.mo_enable, TB_MONITOR.mo_order, TB_MONITOR.mo_delete, TB_MONITOR.mo_phdmi, TB_MONITOR.mo_pvga, TB_MONITOR.mo_pdvi, T_SIZE.si_name, T_SERIES.se_name, T_IMGMNG.img_folder + '/' + T_IMGMNG.img_fname AS imgsrc FROM (((TB_MONITOR INNER JOIN T_IMGMNG ON TB_MONITOR.mo_imgid = T_IMGMNG.img_id) INNER JOIN T_SERIES ON TB_MONITOR.mo_seid = T_SERIES.se_id) INNER JOIN T_SIZE ON TB_MONITOR.mo_siid = T_SIZE.si_id) WHERE (TB_MONITOR.mo_delete = false) AND (TB_MONITOR.mo_enable = true) ORDER BY TB_MONITOR.mo_order DESC">
        </asp:AccessDataSource>
    </div>
    </form>
</body>
